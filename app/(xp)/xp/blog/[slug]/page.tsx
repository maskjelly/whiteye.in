...
export async function generateMetadata({
  ...
  return {
    ...
    openGraph: {
      title: post.title,
      description: post.description,
      url: `https://whiteye.in/xp/blog/${post.slug}`,
      siteName: "aaryan",
      locale: "en_US",
      type: "article",
      ...
      <article>
        <header className="page-header">
          <p className="page-kicker animate-fade-in">
            <Link href="/xp/blog" className="hover:underline">
              writing
            </Link>
          </p>
          ...
          <div className="post-content min-w-0">
            ...
            {prevNext && (
              <nav className="back-row grid grid-cols-2 gap-8">
                {prevNext.prev ? (
                  <Link
                    href={`/xp/blog/${prevNext.prev.slug}`}
                ...
                {prevNext.next ? (
                  <Link
                    href={`/xp/blog/${prevNext.next.slug}`}
            ...
            <div className="back-row flex items-center justify-between">
              <Link href="/xp/blog" className="back-link">
                ← back to writing
              </Link>
